...
window.sexShopOnItemClick = function (index) {
	...
	for (let index in item.colour_options) {
		coloring_div +=
			(index == 0 ? `<br><span style="color: #e0e0e0">고를 수 있는 색상 : </span><div id="ssm_colour_panel" class="colour-options-div"><br> ` : "") + `
...
window.determineRecipient = function(index) { // conditions for gifting items to people
	...
	선물을 받을 사람 :  </a><select name="recipient" id="recipientList">${option_builder}</select>
