...
Object.defineProperty(Array.prototype, 'formatList', {
	...
	value(options) {
		...
		try {if (!useOxfordComma && this.length > 1)  conjunction = (conjunction === "그리고"?["과", "와", "과"]:["이나", "나", "이나"])[getPostNum(this.at(-2))]; conjunction += " "; getPostNum(this.at(-1));	/* useOxfordComma가 false일 때는 조사처리, 아니면 conjunction 사용. 끝으로 출력후의 조사처리를 위해 마지막 글자의 _postNum을 계산해둔다 */} catch(e) { return `${e}: this=${this}, tostring=${this.toString()} `}
